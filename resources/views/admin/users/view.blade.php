...
@section('title')
    {{ trans('admin/general.manage_user_title') }}: {{ $user->username }}
@endsection
...
<div class="row">
        ...
        <div class="col-md-6">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        <label for="registered" class="control-label">{{ trans('strings.username') }}</label>
                        <div>
                            <input type="text" name="username" value="{{ $user->username }}" class="form-control form-autocomplete-stop">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="registered" class="control-label">{{ trans('admin/general.client_first_name') }}</label>
                        <div>
                            <input type="text" name="name_first" value="{{ $user->name_first }}" class="form-control form-autocomplete-stop">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="registered" class="control-label">{{ trans('admin/general.client_last_name') }}</label>
                        <div>
                ...
                <div class="box-footer">
                    {!! csrf_field() !!}
                    {!! method_field('PATCH') !!}
                    <input type="submit" value="{{ trans('admin/general.update_user') }}" class="btn btn-primary btn-sm">
                </div>
            </div>
        </div>
        <div class="col-md-6">
    ...
    <div class="col-xs-12">
        <div class="box box-danger">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('admin/general.delete_user') }}</h3>
            </div>
            <div class="box-body">
                <p class="no-margin">{{ trans('admin/general.delete_user_requirement') }}</p>
            </div>
            <div class="box-footer">
                <form action="{{ route('admin.users.view', $user->id) }}" method="POST">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
                    <input id="delete" type="submit" class="btn btn-sm btn-danger pull-right" {{ $user->servers->count() < 1 ?: 'disabled' }} value="{{ trans('admin/general.delete_user') }}" />
                </form>
            </div>
        </div>
    </div>
