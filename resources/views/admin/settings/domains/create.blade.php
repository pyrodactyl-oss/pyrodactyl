...
@section('title')
  {{ trans('admin/general.create_domain') }}
@endsection
...
@section('content-header')
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">{{ trans('strings.create') }}</li>
  </ol>
@endsection
      ...
      <form action="{{ route('admin.settings.domains.store') }}" method="POST" id="domain-form">
              ...
              <div class="form-group col-md-6">
                ...
                <div>
                  <input type="text" name="name" id="name" class="form-control" value="{{ old('name') }}"
                    placeholder="{{ trans('admin/general.domain_name_example') }}" required />
                ...
                <div>
                  <div class="btn-group" data-toggle="buttons">
                    <label class="btn btn-outline-primary @if(old('is_active', true)) active @endif">
                      <input type="radio" name="is_active" value="1" @if(old('is_active', true)) checked @endif> {{ trans('admin/general.active_label') }}
                    </label>
                    <label class="btn btn-outline-primary @if(!old('is_active', true)) active @endif">
                      <input type="radio" name="is_active" value="0" @if(!old('is_active', true)) checked @endif> {{ trans('admin/general.inactive_label') }}
                    </label>
                  </div>
                ...
                <div>
                  <div class="btn-group" data-toggle="buttons">
                    <label class="btn btn-outline-primary @if(old('is_default', false)) active @endif">
                      <input type="radio" name="is_default" value="1" @if(old('is_default', false)) checked @endif> {{ trans('strings.yes') }}
                    </label>
                    <label class="btn btn-outline-primary @if(!old('is_default', false)) active @endif">
                      <input type="radio" name="is_default" value="0" @if(!old('is_default', false)) checked @endif> {{ trans('strings.no') }}
                    </label>
                  </div>
        ...
        <div class="box box-primary">
          <div class="box-footer">
            ...
            <button type="submit" class="btn btn-sm btn-success pull-right">{{ trans('admin/general.create_domain') }}</button>
          </div>
        </div>
      </form>
    </div>
