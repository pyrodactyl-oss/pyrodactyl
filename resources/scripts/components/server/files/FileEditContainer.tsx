...
const FileEditContainer = () => {
    ...
    return (
        <PageContentBlock className='h-full p-0!' title={action === 'edit' ? `Editing ${filename}` : 'New File'}>
            <FlashMessageRender byKey={'files:view'} />

            <ErrorBoundary>
                <div
                    className={
                        'flex rounded-md rounded-b-none border-[#ffffff07] border-[1px] border-b-0 bg-[#ffffff11] py-6'
                    }
                >
                    <span className='-ml-[2rem]' />
                    <FileManagerBreadcrumbs isNewFile={action !== 'edit'} withinFileEditor />
                </div>
            </ErrorBoundary>

            {['.pyroignore', '.pyroignore'].includes(filename) ? (
                <div className={'mb-4 rounded-sm border-cyan-400 border-l-4 bg-neutral-900 p-4'}>
                    <p className={'text-neutral-300 text-sm'}>
                        You&apos;re editing a{' '}
                        <code className={'rounded-sm bg-black px-1 py-px font-mono'}>.pyroignore</code> file. Any files
                        or directories listed in here will be excluded from backups. Wildcards are supported by using an
                        asterisk (<code className={'rounded-sm bg-black px-1 py-px font-mono'}>*</code>
                        ). You can negate a prior rule by prepending an exclamation point (
                        <code className={'rounded-sm bg-black px-1 py-px font-mono'}>!</code>
                        ).
                    </p>
                </div>
            ) : null}
            ...
            <div className='relative h-full w-full flex-grow border-[#ffffff07] border-[1px] border-t-0 bg-[#ffffff11] [&>div>div]:h-full [&>div>div]:outline-hidden!'>
                <Editor
            ...
            <div className='absolute top-2.5 right-2 flex flex-row items-center gap-4'>
                <DropdownMenu>
                    <DropdownMenuTrigger className='flex h-fit items-center gap-2 rounded-md bg-[#ffffff11] px-3 py-1 font-bold text-sm'>
                        ...
                        <span className='hidden sm:block'>{language?.name ?? 'Language'}</span>
                    ...
                    <DropdownMenuContent className='z-99999 max-h-50 overflow-auto' sideOffset={8}>
                ...
                {action === 'edit' ? (
                    <Can action={'file.update'}>
                        <div className='flex items-center justify-center gap-1'>
                            <ActionButton
                                className='rounded-r-none rounded-l-full pr-6 pl-8'
                                onClick={() => save()}
                                size='lg'
                                variant='primary'
                            >
                                Save{' '}
                                <span className='ml-2 hidden font-bold font-mono text-xs uppercase lg:inline-block'>
                                    CTRL + S
                                </span>
                            </ActionButton>
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent
                                    className='z-99999 max-h-[calc(100vh-4rem)] overflow-auto'
                                    sideOffset={8}
                                >
                    ...
                    <Can action={'file.create'}>
                        <ActionButton onClick={() => setModalVisible(true)} size='lg' variant='secondary'>
                            Create File
                        </ActionButton>
                    </Can>
                )}
