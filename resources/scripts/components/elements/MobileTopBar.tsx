...
const MobileTopBar = ({ onMenuToggle, onTriggerLogout, onSelectAdminPanel, rootAdmin }: MobileTopBarProps) => {
    ...
    return (
        <div className='fixed top-0 right-0 left-0 z-50 flex h-16 items-center justify-between border-[#ffffff08] border-b bg-[#1a1a1a] px-4 lg:hidden'>
            {/* Logo */}
            ...
            <div className='flex items-center gap-2'>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className='flex h-10 w-10 cursor-pointer items-center justify-center rounded-md p-2 text-white hover:bg-[#ffffff11]'>
                            <svg
                ...
                <button
                    ...
                    className='flex h-10 w-10 cursor-pointer items-center justify-center rounded-md p-2 text-white hover:bg-[#ffffff11]'
                    onClick={handleMenuToggle}
                >
                    <BarsPlay fill='currentColor' height={22} width={22} />
                </button>
            </div>
        </div>
    );
