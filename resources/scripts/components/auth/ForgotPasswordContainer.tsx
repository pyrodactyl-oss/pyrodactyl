...
const ForgotPasswordContainer = () => {
    ...
    const handleSubmission = ({ email }: Values, { setSubmitting, resetForm }: FormikHelpers<Values>) => {
        ...
        http.post('/auth/password', requestData)
            .then((response) => {
                resetForm();
                addFlash({ type: 'success', title: i18n.t('strings:success'), message: response.data.status || i18n.t('auth:forgot.success') });
            })
            .catch((error) => {
                console.error(error);
        ...
        <ContentBox>
            <Formik
                ...
                validationSchema={object().shape({
                    email: string().email(i18n.t('auth:forgot.email_valid')).required(i18n.t('auth:forgot.email_required')),
                })}
            >
                {({ isSubmitting }) => (
                    <LoginFormContainer className={`w-full flex`}>
                        ...
                        <Captcha className='mt-6' onError={() => {
                            addFlash({ type: 'error', title: i18n.t('strings:error'), message: i18n.t('auth:captcha_failed') });
                        }} />
                        <div className='mt-6'>
                            <Button className={`w-full mt-4 rounded-full bg-brand border-0 ring-0 outline-hidden capitalize font-bold text-sm py-2`} type='submit' size='xlarge' isLoading={isSubmitting} disabled={isSubmitting}>
                                {t('forgot.send_email')}
                            </Button>
                        </div>
                        <div aria-hidden className='my-8 bg-[#ffffff33] min-h-[1px]'></div>
