...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = trans('exceptions.daemon.connection_failed');
        } else {
            $message = sprintf(trans('exceptions.daemon.communication_error'), $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf(trans('exceptions.daemon.remote_error'), $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
