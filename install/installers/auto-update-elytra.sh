...
######################################################################################
#                                                                                    #
# Hydrodactyl Elytra Auto-Updater (Docker)                                           #
#                                                                                    #
# Pulls the latest Elytra image and restarts the service.                            #
#                                                                                    #
######################################################################################

ELYTRA_REPO="${ELYTRA_REPO:-pyrohost/elytra}"
COMPOSE_DIR="${COMPOSE_DIR:-/opt/hydrodactyl}"
LOG_FILE="${LOG_FILE:-/var/log/hydrodactyl-elytra-update.log}"
...
check_for_updates() {
  log "Checking for new Elytra Docker image: ghcr.io/$ELYTRA_REPO:latest"

  local old_id new_id
  old_id=$(docker image inspect "ghcr.io/$ELYTRA_REPO:latest" --format '{{.Id}}' 2>/dev/null || echo "none")

  if ! docker pull "ghcr.io/$ELYTRA_REPO:latest" 2>/dev/null; then
    log "ERROR: Failed to pull image"
    return 1
  fi

  new_id=$(docker image inspect "ghcr.io/$ELYTRA_REPO:latest" --format '{{.Id}}' 2>/dev/null)
...
main() {
  while [[ $# -gt 0 ]]; do case $1 in
    ...
    --help|-h) echo "Hydrodactyl Elytra Docker Auto-Updater"; exit 0 ;;
