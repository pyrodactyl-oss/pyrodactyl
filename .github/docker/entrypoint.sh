...
(
  ...
  if [ "$PYRODACTYL_DOCKER_DEV" = "true" ] && [ "$DEV_SETUP" != "true" ]; then
    ...
    export POSTGRES_PASSWORD=$(grep "POSTGRES_PASSWORD" compose.develop.yml | awk '{print $2}')
    export POSTGRES_USER=$(grep "POSTGRES_USER" compose.develop.yml | awk '{print $2}')
